...
class Ajax_pagination{
	...
	var $first_tag_close	= '';
	var $last_tag_open	= '';
	var $last_tag_close	= '</ul>';
	var $cur_tag_open	= '<li class="page-item">';
	...
	function create_links(){
		...
		if ($num_pages == 1){
            		$info = '<div class="pagination"><span>Showing : ' . $this->total_rows.'</span></div>';
			return $info;
		}
		...
		for ($loop = $start -1; $loop <= $end; $loop++){
			...
			if ($i >= 0){
				if ($this->cur_page == $loop){
					$output .= $this->cur_tag_open.'<a href="javascript:void(0);" ' . $this->anchor_class . ' id="active"><b>'.$loop.'</b></a>'.$this->cur_tag_close; // Current page
				}else{
					$n = ($i == 0) ? '' : $i;
